    ...
    private fun drawPerspective(canvas: Canvas, w: Float, h: Float, skipEffects: Boolean = false, bounds: RectF) {
        ...
        if (bmpW > 0 && bmpH > 0) {
            ...
            if (morphedBmpCache == null || morphedBmpCache!!.isRecycled || morphedBmpCache!!.width != targetBmpW || morphedBmpCache!!.height != targetBmpH || morphedBmpHash != shapeHash) {
                ...
                tempCanvasSolid.translate(-w / 2f, -h / 2f)
                val prevSilhouette = silhouetteColor
                silhouetteColor = Color.BLACK
                try {
                    drawContent(tempCanvasSolid, w, h, skipEffects = skipEffects)
                } finally {
    ...
    private fun drawWarped(canvas: Canvas, w: Float, h: Float, rows: Int, cols: Int, mesh: FloatArray, qualityScale: Float = 1.0f, skipEffects: Boolean = false, bounds: RectF, viewScale: Float = 1.0f) {
        ...
        if (bmpW > 0 && bmpH > 0) {
            ...
            if (morphedBmpCache == null || morphedBmpCache!!.isRecycled || morphedBmpCache!!.width != targetBmpW || morphedBmpCache!!.height != targetBmpH || morphedBmpHash != shapeHash) {
                ...
                val solidC = Canvas(solidBmp)
                solidC.scale(qualityScale, qualityScale)
                solidC.translate(pad, pad)
                val prevSilhouette = silhouetteColor
                silhouetteColor = Color.BLACK
                try {
                    drawContent(solidC, w, h, skipEffects = skipEffects, viewScale = viewScale)
                } finally {
                    silhouetteColor = prevSilhouette
                }
                ...
                tempCanvasSolid.scale(qualityScale, qualityScale)
                tempCanvasSolid.translate(-bounds.left, -bounds.top)
                tempCanvasSolid.drawBitmapMesh(solidBmp, meshW, meshH, paddedVerts, 0, null, 0, tempPaint)
                solidBmp.recycle()
                solidMorphedBmpCache = solidMorphedBmp
                solidMorphedBmpHash = shapeHash
            }
