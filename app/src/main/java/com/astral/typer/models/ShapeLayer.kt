...
class ShapeLayer(
    ...
    private fun drawPerspective(canvas: Canvas, w: Float, h: Float, skipEffects: Boolean = false, bounds: RectF, viewScale: Float = 1.0f) {
    ...
    private fun drawWarped(canvas: Canvas, w: Float, h: Float, rows: Int, cols: Int, mesh: FloatArray, qualityScale: Float = 1.0f, skipEffects: Boolean = false, bounds: RectF, viewScale: Float = 1.0f) {
        ...
        if (bmpW > 0 && bmpH > 0) {
            ...
            val shapeHash = listOf(shapeName, w, h, color, warpRows, warpCols, warpMesh?.contentHashCode() ?: 0, perspectivePoints?.contentHashCode() ?: 0, qualityScale, (viewScale >= 0.2f), isRoughStroke, roughStrokeRoughness).hashCode()
            if (morphedBmpCache == null || morphedBmpCache!!.isRecycled || morphedBmpCache!!.width != targetBmpW || morphedBmpCache!!.height != targetBmpH || morphedBmpHash != shapeHash) {
                recycleMorphedCaches()
                val morphedBmp = Bitmap.createBitmap(targetBmpW, targetBmpH, Bitmap.Config.ARGB_8888)
                val tempCanvas = Canvas(morphedBmp)
                ...
                if (hasWoodScratch) {
                    ...
                    if (!useRenderEffect) {
                        ...
                        TextLayer.drawWoodScratchSoftware(
                            ...
                            woodScratchIntensity, woodScratchSeed, woodScratchColor, drawInnerHalftoneContent
                        )
                    }
                } else {
    ...
    private fun getMultiGradientShader(w: Float, h: Float): Shader {
        ...
        val positions = if (multiGradientPositions != null && multiGradientPositions!!.size == multiGradientColors.size) {
            ...
            FloatArray(multiGradientColors.size) { i -> i.toFloat() / (multiGradientColors.size - 1) }
        }
