    ...
    private fun drawPerspective(canvas: Canvas, layout: StaticLayout, w: Float, h: Float, ch: Float, skipEffects: Boolean = false, bounds: RectF, viewScale: Float = 1.0f) {
        ...
        if (bmpW > 0 && bmpH > 0) {
            ...
            if (morphedBmpCache == null || morphedBmpCache!!.isRecycled || morphedBmpCache!!.width != targetBmpW || morphedBmpCache!!.height != targetBmpH || morphedBmpHash != shapeHash) {
                ...
                // Generate solid silhouette cache
                val solidFinalBmp = Bitmap.createBitmap(bmpW, bmpH, Bitmap.Config.ARGB_8888)
                val solidCanvas = Canvas(solidFinalBmp)
                solidCanvas.scale(qualityScale, qualityScale)
                solidCanvas.translate(pad, pad)
                val prevSil = silhouetteColor
                silhouetteColor = Color.BLACK
                try {
                    drawCleanContent(solidCanvas, layout, w, ch, skipEffects = skipEffects, viewScale = viewScale)
                    if (eraseMask != null) {
                        val maskPaint = Paint(Paint.ANTI_ALIAS_FLAG).apply { xfermode = PorterDuffXfermode(PorterDuff.Mode.DST_OUT) }
                        solidCanvas.drawBitmap(eraseMask!!, null, RectF(0f, 0f, bmpW.toFloat(), bmpH.toFloat()), maskPaint)
                    }
                } finally {
                    silhouetteColor = prevSil
                }

                val solidMorphedBmp = Bitmap.createBitmap(targetBmpW, targetBmpH, Bitmap.Config.ARGB_8888)
                val solidTempCanvas = Canvas(solidMorphedBmp)
                solidTempCanvas.scale(qualityScale, qualityScale)
                solidTempCanvas.translate(-bounds.left, -bounds.top)
                solidTempCanvas.save()
                solidTempCanvas.concat(matrix)
                solidTempCanvas.drawBitmap(solidFinalBmp, null, destRect, tempPaint)
            ...
            if (hasStrokes) {
                ...
                if (fillAlpha > 0) {
                    val erasePaint = Paint(Paint.ANTI_ALIAS_FLAG).apply {
                        ...
                        val alphaMultiplier = fillAlpha / 255f
                        val cm = android.graphics.ColorMatrix(floatArrayOf(
                            1f, 0f, 0f, 0f, 0f,
                            0f, 1f, 0f, 0f, 0f,
                            0f, 0f, 1f, 0f, 0f,
    ...
    private fun drawWarped(canvas: Canvas, layout: StaticLayout, w: Float, h: Float, ch: Float, rows: Int, cols: Int, mesh: FloatArray, qualityScale: Float = 1.0f, skipEffects: Boolean = false, bounds: RectF, viewScale: Float = 1.0f) {
        ...
        if (bmpW > 0 && bmpH > 0) {
            ...
            if (morphedBmpCache == null || morphedBmpCache!!.isRecycled || morphedBmpCache!!.width != targetBmpW || morphedBmpCache!!.height != targetBmpH || morphedBmpHash != shapeHash) {
                ...
                // Generate solid silhouette cache
                val solidFinalBmp = if (isFreshBmp) {
                    val solidBmp = Bitmap.createBitmap(bmpW, bmpH, Bitmap.Config.ARGB_8888)
                    val solidCanvas = Canvas(solidBmp)
                    solidCanvas.scale(qualityScale, qualityScale)
                    solidCanvas.translate(-srcBounds.renderLeft, -srcBounds.renderTop)
                    val prevSil = silhouetteColor
                    silhouetteColor = Color.BLACK
                    try {
                        drawCharacterByCharacter(solidCanvas, layout, w, h, ch, qualityScale, skipEffects = skipEffects, viewScale = viewScale)
                    } finally {
                        silhouetteColor = prevSil
                    }
                    solidBmp
                ...
                solidTempCanvas.translate(-bounds.left, -bounds.top)
                if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.Q) {
                    solidTempCanvas.drawBitmapMesh(solidFinalBmp, meshW, meshH, paddedVerts, 0, null, 0, tempPaint)
                } else {
                    solidTempCanvas.drawBitmapMesh(solidFinalBmp, meshW, meshH, paddedVerts, 0, null, 0, null)
                }
                solidFinalBmp.recycle()
