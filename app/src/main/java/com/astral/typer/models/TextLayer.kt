...
class TextLayer(
    ...
    private fun drawPerspective(canvas: Canvas, layout: StaticLayout, w: Float, h: Float, ch: Float, skipEffects: Boolean = false) {
        ...
        if (bmpW > 0 && bmpH > 0) {
            ...
            // Render perspective content into morphedBmp
            ...
            if (hasStrokes) {
                ...
                if (tripleStrokeWidth > 0f && doubleStrokeWidth > 0f) {
                    ...
                    val blurredAlphaBmp = morphedBmp.extractAlpha(blurPaint, offset)
                    if (blurredAlphaBmp != null) {
                        ...
                        canvas.drawBitmap(blurredAlphaBmp, offset[0].toFloat(), offset[1].toFloat(), strokePaint)
                        blurredAlphaBmp.recycle()
                    }
                }
            ...
            canvas.drawBitmap(morphedBmp, 0f, 0f, paint)
            morphedBmp.recycle()
        }
    }
        ...
        for (i in 0 until fullText.length) {
            ...
            if (mesh != null) {
                ...
                if (bmpW > 0 && bmpH > 0) {
                    ...
                    val paint = Paint(Paint.ANTI_ALIAS_FLAG).apply { isFilterBitmap = true }
                    val drawCharMesh = { p: Paint? ->
                        if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.Q) {
                            canvas.drawBitmapMesh(tempBmp, meshW, meshH, paddedVerts, 0, null, 0, p ?: paint)
                        } else {
                            canvas.drawBitmapMesh(tempBmp, meshW, meshH, paddedVerts, 0, null, 0, p)
                        }
                    }
    ...
    private fun drawWarped(canvas: Canvas, layout: StaticLayout, w: Float, h: Float, ch: Float, rows: Int, cols: Int, mesh: FloatArray, qualityScale: Float = 1.0f, skipEffects: Boolean = false) {
        ...
        if (bmpW > 0 && bmpH > 0) {
            ...
            // Render the warped mesh into morphedBmp offscreen
            val morphedBmp = Bitmap.createBitmap(bmpW, bmpH, Bitmap.Config.ARGB_8888)
            val tempCanvas = Canvas(morphedBmp)
            val tempPaint = Paint(Paint.ANTI_ALIAS_FLAG).apply { isFilterBitmap = true }
            if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.Q) {
